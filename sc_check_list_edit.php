<?php 
	session_start();
		if($_SESSION['user_role_id']!= 5 && $_SESSION['user_role_id']!= 2)
	{
			...
			<div class="row">			
						<?php
						  ...
						  $strSQL  = oci_parse($objConnect, 
						                       "SELECT ID, REF_CODE, CURRENT_PARTY_NAME, 
												...
												FROM RML_COLL_SC_CCD where id=$sc_id and CCD_APPROVAL_STATUS=1"); 
	              ...
	              <?php
						  ...
						  if(isset($_POST['ref_code'])){
                              ...
                              @$check_list_1 = $_REQUEST['check_list_1'];
                              @$check_list_2 = $_REQUEST['check_list_2'];
                              @$check_list_3 = $_REQUEST['check_list_3'];
                              @$check_list_4 = $_REQUEST['check_list_4'];
                              @$check_list_5 = $_REQUEST['check_list_5'];
                              @$check_list_6 = $_REQUEST['check_list_6'];
                              @$check_list_7 = $_REQUEST['check_list_7'];
                              @$check_list_8 = $_REQUEST['check_list_8'];
                              @$check_list_9 = $_REQUEST['check_list_9'];
                              @$file_status = $_REQUEST['file_status'];
							  $emp_session_id=$_SESSION['emp_id'];
							 
							 if($file_status=='update'){
								 $strSQL  = oci_parse($objConnect, 
								             "update RML_COLL_SC_CCD_CHECKLIST SET 
											        STATUS=1,
													APPROVAL_DATE=SYSDATE,
													PROBLEM_STATUS=1,
													APPROVED_BY='$emp_session_id'
												where LIST_ID IN (
												             '$check_list_1',
															 '$check_list_2',
															 '$check_list_3',
												...
												and RML_COLL_SC_CCD_ID='$sc_id'
												and STATUS IS NULL");  
							  }else if($file_status=='clear'){
								 $strSQL  = oci_parse($objConnect, "UPDATE RML_COLL_SC_CCD SET
																	FILE_CLEAR_STATUS=1,
																	FILE_CLEAR_DATE=SYSDATE,
																	FILE_CLEAR_BY='$emp_session_id'
													 WHERE ID=$sc_id");   
							  }else if($file_status=='reissues'){
								 $strSQL  = oci_parse($objConnect, "UPDATE RML_COLL_SC_CCD SET
																	FILE_CLEAR_STATUS=0,
																	FILE_CLEAR_DATE=SYSDATE,
																	FILE_CLEAR_BY='$emp_session_id'
                               ...
                               /* $strSQLReissue  = oci_parse($objConnect, 
								                   "INSERT INTO RML_COLL_SC_CCD_CHECKLIST_FAIL (
                                                      RML_COLL_SC_CCD_ID, 
													  REF_CODE, 
                                                      STATUS, 
													  ISSUED_DATE, 
                                ...
                                */
                                $strSQLReissue  = oci_parse($objConnect, 
								                   "BEGIN
												    SC_FILEPROBLEM_CREATE($sc_id,'$ref_code','$emp_session_id');
												    END;
												   "); 								
						      ...
						      if(oci_execute($strSQL)){
								 echo $htmlHeader;
									while($stuff)
									{
									 ...
									 echo "<script>window.location = 'http://202.40.181.98:9090/rangs_collection_rml/sc_check_list_edit.php?sc_id=$sc_id'</script>"; 
