	    ...
	    if($USER_ROLE=="ALL"){
	     ...
	     // For visit
	      ...
	      $AMISHEstrSQL  = oci_parse($objConnect, "Select SUM(VISIT_UNIT) ASSIGN_VISIT,SUM(TOTAL_VISIT) NON_PAYMENT_VISIT,SUM(EMI_VISIT) PAYMENT_VISIT FROM
			...
			// concerrn
			 // Amishe
	      $CONCERNstrSQL  = oci_parse($objConnect, "select distinct(R_CONCERN) R_CONCERN,count(R_CONCERN) TOTAL from RML_HR_APPS_USER
													group by R_CONCERN"); 
			...
			if(oci_execute($CONCERNstrSQL)) {
			   while($row=oci_fetch_assoc($CONCERNstrSQL)){ 
					$R_CONCERN = $row['R_CONCERN'];
					if($R_CONCERN=='RML'){
					  $RML_TOTAL=$row['TOTAL'];
					}else if($R_CONCERN=='RG'){
					  $RG_TOTAL=$row['TOTAL'];
					}else if($R_CONCERN=='AMS'){
					  $AMISHE_TOTAL=$row['TOTAL'];
					}
			    }
			}
