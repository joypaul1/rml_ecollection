			...
			<div class="row">
						...
						if(isset($_POST['ref_code'])){
						    ...
						    $strSQL  = oci_parse($objConnect, 
						             "SELECT 
										   ...
										   A.FILE_CLEAR_STATUS
										FROM RML_COLL_SC_CCD A,RML_COLL_CCD_SC_HANDOVER B
										where A.ID=B.RML_COLL_SC_CCD_ID
										AND A.FILE_CLEAR_STATUS=1
										AND B.SC_CAR_STATUS IS NULL
						 ...
						 <?php
						   ...
						   <tr>
							<td></td>
							<td>
							<input class="btn btn-primary btn pull-right" type="submit" name="submit_approval" value="SC Customer Received"/>	
							</td>
							<td></td>
							<td></td>
							<td></td>
				...
				<?php
					...
					foreach($_POST['check_list'] as $TT_ID_SELECTTED){
						$strSQL = oci_parse($objConnect, 
					                        "update RML_COLL_CCD_SC_HANDOVER set 
												SC_CAR_STATUS=1,
												SC_CAR_BY='$emp_session_id',
												SC_CAR_DATE=SYSDATE
											   where ID='$TT_ID_SELECTTED'");
						
						oci_execute($strSQL);
											
					//echo 'Successfully Approved ID '.$TT_ID_SELECTTED."</br>";
					...
					while($stuff)
					{
					 ...
					 echo "<script>window.location = 'http://202.40.181.98:9090/rangs_collection_rml/sc_customer_received.php'</script>";
